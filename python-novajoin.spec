...
install -p -D -m 640 files/join.conf %{buildroot}%{_sysconfdir}/%{service}/join.conf
install -p -D -m 644 %{buildroot}%{python3_sitelib}%{_sysconfdir}/%{service}/cloud-config-novajoin.json \
  %{buildroot}%{_sysconfdir}/%{service}/cloud-config-novajoin.json
install -p -D -m 644 %{buildroot}%{python3_sitelib}%{_sysconfdir}/%{service}/join-api-paste.ini \
  %{buildroot}%{_sysconfdir}/%{service}/join-api-paste.ini
install -p -D -m 755 %{buildroot}%{python3_sitelib}%{_libexecdir}/novajoin-ipa-setup \
  %{buildroot}%{_libexecdir}/novajoin-ipa-setup
rm -f %{buildroot}%{python3_sitelib}%{_sysconfdir}/%{service}/cloud-config-novajoin.json
rm -f %{buildroot}%{python3_sitelib}%{_sysconfdir}/%{service}/join-api-paste.ini
rm -f %{buildroot}%{python3_sitelib}%{_libexecdir}/novajoin-ipa-setup

# install data file
install -p -D -m 644 %{buildroot}%{python3_sitelib}%{_datadir}/%{service}/freeipa.json \
  %{buildroot}%{_datadir}/%{service}/freeipa.json
rm -f %{buildroot}%{python3_sitelib}%{_datadir}/%{service}/freeipa.json

# install log rotation configuration
install -p -D -m 644 -p %{SOURCE1} \
  %{buildroot}%{_sysconfdir}/logrotate.d/%{service}

# install man pages
install -p -D -m 644 %{buildroot}%{python3_sitelib}%{_mandir}/man1/novajoin* \
  %{buildroot}%{_mandir}/man1/
rm -f %{buildroot}%{python3_sitelib}%{_mandir}/man1/novajoin*

# Install the upstream-provided systemd unit files
install -p -D -m 644 %{buildroot}%{python3_sitelib}%{_datarootdir}/%{service}/novajoin-notify.service \
  %{buildroot}%{_unitdir}/novajoin-notify.service
install -p -D -m 644 %{buildroot}%{python3_sitelib}%{_datarootdir}/%{service}/novajoin-server.service \
  %{buildroot}%{_unitdir}/novajoin-server.service
rm -f %{buildroot}%{python3_sitelib}%{_datarootdir}/%{service}/novajoin-notify.service
rm -f %{buildroot}%{python3_sitelib}%{_datarootdir}/%{service}/novajoin-server.service
